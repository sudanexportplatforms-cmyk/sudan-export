...
export default function RfqDetail() {
  ...
  if (rfqLoading) {
    return (
      <PortalLayout role="buyer" title="RFQ Details">
  ...
  if (!rfq) {
    return (
      <PortalLayout role="buyer" title="RFQ Not Found">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-none shadow-sm">
            <CardHeader className="pb-4 border-b border-gray-100">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription className="text-sm">Posted on {format(new Date(rfq.createdAt), 'MMMM d, yyyy')}</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              ...
              {rfq.notes && (
                <div className="mb-8">
                  <h3 className="text-sm font-bold text-gray-900 mb-2">Additional Notes</h3>
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-900 mb-4">Requested Items</h3>
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>
                        ...
                        <th className="px-4 py-3 font-medium text-gray-500">Target Price</th>
                      </tr>
                    </thead>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="border-none shadow-sm bg-primary text-primary-foreground">
            <CardContent className="p-6 text-center">
              <h3 className="text-lg font-semibold mb-2">Quotations Received</h3>
              <p className="text-5xl font-bold mb-4">{quotations.length}</p>
              <p className="text-sm text-primary-foreground/80">Review quotes below and award the deal.</p>
            </CardContent>
          </Card>
        </div>
      </div>

      <h2 className="text-xl font-bold text-gray-900 mb-4">Quotation Comparison</h2>
      ...
      {quotesLoading ? (
        ...
        <Card className="border-dashed border-2 shadow-none bg-transparent">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-500 max-w-sm">Suppliers have not yet submitted quotes for this RFQ. You'll be notified when they do.</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardHeader className={`pb-4 ${quote.status === 'awarded' ? 'bg-[#C9A24A]/5' : quote.status === 'shortlisted' ? 'bg-primary/5' : ''}`}>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription className="mt-1 text-xs">Submitted {format(new Date(quote.createdAt), 'MMM d, yyyy')}</CardDescription>
                  </div>
              ...
              <CardContent className="pt-4 flex-1">
                ...
                {quote.items && quote.items.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <p className="text-xs font-semibold text-gray-900 mb-2 uppercase tracking-wide">Item Breakdown</p>
                    <div className="space-y-2">
                      {quote.items.map((item, idx) => (
                        <div key={idx} className="flex justify-between text-sm">
                          <span className="text-gray-600 line-clamp-1 flex-1 pr-2" title={item.productName}>
              ...
              {rfq.status !== 'closed' && rfq.status !== 'cancelled' && (
                <div className="p-4 border-t border-gray-100 bg-gray-50/50 mt-auto flex flex-wrap gap-2">
                  {quote.status !== 'awarded' && quote.status !== 'rejected' && (
                    ...
                    >
                      <CheckCircle className="w-4 h-4 mr-1.5" /> Award
                    </Button>
                  )}
